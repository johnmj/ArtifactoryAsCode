...
# This script is not to be invoked directly. Should be triggered by "startup.sh"
...
-l   : Specify path to artifactory.lic file ("Pro" only)
-h   : Show this usage
...
processOptions() {
    while getopts ":t:d:l:cfh" opt; do
        case $opt in
            t)  # Run type
                TYPE=$OPTARG
...
copyFiles () {
    ...
    cp -fr ${SCRIPT_DIR}/nginx/conf.d/* ${ROOT_DATA_DIR}/nginx/conf.d/
}
...
======================================
IMPORTANT
* Artifactory Pro - The license key would automatically placed in "{ROOT_DATA_DIR}/artifactory/etc" if the license path is provided while executing this script. Else it will have to be manually placed later before starting Artifactory.
  Artifactory HA - Before starting, it is recommended to place the license file(s) (artifactory.lic) in the Artifactory etc directory
...
validateSudo
processOptions $*
## The below function calls are to be enabled only on the first run or if the existing installation is to be overwritten;
#cleanDataDir
#createDirectories
copyFiles
showNotes
